...
static int do_mqtt_cmd(int argc, char **argv)
{
    ...
    if (mqtt_args.connect->count > 0) {
        ...
        if (mqtt_args.uri->count > 0) {
            uri = strdup(mqtt_args.uri->sval[0]);
        } else {
            uri = strdup(CONFIG_MQTT_BROKER_URL);
        }
...
esp_err_t set_mqtt_server_certs(const uint8_t *ca_cert_pem_start_i, const uint8_t *ca_cert_pem_end_i)
{
...
esp_err_t console_cmd_mqtt_register(void)
{
    ...
    mqtt_args.cert = arg_lit0(NULL, "cert", "Define the PEM encoded certificate for this client, if required by the server (flag, no argument)");
    mqtt_args.key = arg_lit0(NULL, "key", "Define the PEM encoded private key for this client, if required by the server (flag, no argument)");
