...
static const uint8_t *client_cert_pem_start = NULL;
static const uint8_t *client_cert_pem_end = NULL;
static const uint8_t *client_key_pem_start = NULL;
static const uint8_t *client_key_pem_end = NULL;
static const uint8_t *server_cert_pem_start = NULL;
static const uint8_t *server_cert_pem_end = NULL;
                         ...
                         const uint8_t *server_cert_pem_start_i, const uint8_t *server_cert_pem_end_i)
{
    ...
    if (client_key_pem_start_i && client_key_pem_end_i &&
        ...
        ESP_LOGE(TAG, "Invalid mqtt cert key(%d)\n", __LINE__);
        return ESP_ERR_INVALID_ARG;
    }

    if (!server_cert_pem_start_i || !server_cert_pem_end_i ||
            (server_cert_pem_start_i > server_cert_pem_end_i)) {
        ESP_LOGE(TAG, "Invalid mqtt certs(%d)\n", __LINE__);
        return ESP_ERR_INVALID_ARG;
    }

    client_cert_pem_start = client_cert_pem_start_i;
    client_cert_pem_end = client_cert_pem_end_i;
    client_key_pem_start = client_key_pem_start_i;
    client_key_pem_end = client_key_pem_end_i;
    server_cert_pem_start = server_cert_pem_start_i;
    server_cert_pem_end = server_cert_pem_end_i;
...
esp_err_t console_cmd_mqtt_register(void)
{
    ...
    mqtt_args.cert = arg_lit0("", "cert", "Define the PEM encoded certificate for this client, if required by the server");
    mqtt_args.key = arg_lit0("", "key", "Define the PEM encoded private key for this client, if required by the server");
    mqtt_args.cafile = arg_lit0("", "cafile", "Define the PEM encoded CA certificates that are trusted");
