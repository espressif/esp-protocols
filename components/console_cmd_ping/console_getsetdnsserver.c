...
static void print_available_ifaces(void)
{
    ...
    for (esp_netif_t *esp_netif = netif_next(NULL); esp_netif != NULL; esp_netif = netif_next(esp_netif)) {
        ...
        if (esp_netif_get_netif_impl_name(esp_netif, impl) == ESP_OK) {
            printf("  %s  (%s)\n", impl, if_key ? if_key : "?");
        } else {
            printf("  ?  (%s)\n", if_key ? if_key : "?");
        }
...
/* name is impl name (st2) or if_key (WIFI_STA_DEF). Impl name wins. */
static esp_netif_t *resolve_netif(const char *name)
{
    if (name == NULL || name[0] == '\0') {
        printf("Unknown interface '%s'\n", name ? name : "");
        print_available_ifaces();
        return NULL;
    }
    ...
    for (esp_netif_t *esp_netif = netif_next(NULL); esp_netif != NULL; esp_netif = netif_next(esp_netif)) {
        char impl[10] = {0};
        if (esp_netif_get_netif_impl_name(esp_netif, impl) != ESP_OK) {
            continue;  /* not attached yet; do not abort the walk */
        }
        if (strcmp(impl, name) == 0) {
            return esp_netif;
        }
    }

    esp_netif_t *by_key = esp_netif_get_handle_from_ifkey(name);
    if (by_key == NULL) {
        printf("Unknown interface '%s'\n", name);
        print_available_ifaces();
    }
...
static int do_setdnsserver_cmd(int argc, char **argv)
{
    ...
    if (setdnsserver_args.ifkey->count > 0) {
        target = resolve_netif(setdnsserver_args.ifkey->sval[0]);
        if (target == NULL) {
            return 1;
        }
#ifndef CONFIG_ESP_NETIF_SET_DNS_PER_DEFAULT_NETIF
        /* Off by default, and absent before IDF 5.4. Client DNS is one shared
         * lwIP table, so --if does not isolate the write. DHCP-server DNS is
         * still stored on that netif. */
        if ((esp_netif_get_flags(target) & ESP_NETIF_DHCP_SERVER) == 0) {
            printf("setdnsserver: warning: CONFIG_ESP_NETIF_SET_DNS_PER_DEFAULT_NETIF is disabled; "
                   "--if updates the shared lwIP DNS table, not only this interface\n");
        }
#endif
    }
...
esp_err_t console_cmd_setdnsserver_register(void)
{
    ...
    setdnsserver_args.ifkey = arg_str0(NULL, "if", "<iface>", "Set only this interface (impl name or if_key, e.g. st2 or WIFI_STA_DEF)");
    ...
    const esp_console_cmd_t setdnsserver_cmd = {
        ...
        .help = "Usage: setdnsserver [--if <iface>|--global] <main> [backup] [fallback]",
        .hint = NULL,
...
static int do_getdnsserver_cmd(int argc, char **argv)
{
    ...
    esp_netif_t *esp_netif = resolve_netif(filter);
    if (esp_netif == NULL) {
        return 1;
    }
    print_netif_dns(esp_netif, default_netif);
...
esp_err_t console_cmd_getdnsserver_register(void)
{
    ...
    getdnsserver_args.target = arg_str0(NULL, NULL, "iface|global", "Print only this table: 'global', impl name (e.g. st2), or if_key (e.g. WIFI_STA_DEF).");
    getdnsserver_args.end = arg_end(1);
    const esp_console_cmd_t getdnsserver_cmd = {
        .command = "getdnsserver",
        .help = "Usage: getdnsserver [iface|global]",
        .hint = NULL,
