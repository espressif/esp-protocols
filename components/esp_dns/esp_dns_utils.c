...
void esp_dns_parse_response(uint8_t *buffer, size_t response_size, dns_response_t *dns_response, u8_t max_ips)
{
    ...
    if (max_ips == 0) {
        dns_response->status_code = ERR_ARG;
        return;
    }
    ...
    for (int i = 0; i < scan_count; i++) {
        if (dns_response->num_answers >= max_ips) {
            break;
        }
...
{
    if (ipaddr == NULL || max_entries == 0) {
        return ERR_ARG;
    }

    memset(ipaddr, 0, max_entries * sizeof(ip_addr_t));
    ...
    u8_t copy_count = (u8_t)response->num_answers;
    if (copy_count > max_entries) {
        copy_count = max_entries;
    }

    for (u8_t i = 0; i < copy_count; i++) {
