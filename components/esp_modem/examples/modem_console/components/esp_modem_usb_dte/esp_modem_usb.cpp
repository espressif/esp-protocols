...
public:
    explicit UsbTerminal(const esp_modem_dte_config *config)
    {
        ...
        if (usb_config->install_usb_host) {
            ...
            ESP_MODEM_THROW_IF_ESP_FAIL(usb_host_install(&host_config), "USB Host install failed");
        ...
        if (usb_config->cdc_compliant) {
            ESP_MODEM_THROW_IF_ESP_FAIL(this->CdcAcmDevice::open(usb_config->vid, usb_config->pid,
                            usb_config->interface_idx, &esp_modem_cdc_acm_device_config),
                            "USB Device open failed");
        } else {
            ESP_MODEM_THROW_IF_ESP_FAIL(this->CdcAcmDevice::open_vendor_specific(usb_config->vid, usb_config->pid,
                            usb_config->interface_idx, &esp_modem_cdc_acm_device_config),
                            "USB Device open failed");
        }
    };
