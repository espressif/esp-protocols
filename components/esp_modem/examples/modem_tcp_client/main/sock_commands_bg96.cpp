...
Responder::ret Responder::recv(uint8_t *data, size_t len)
{
    ...
    if (data_to_recv == 0) {
        ...
        // GCC 15 and later require the 5-argument version of std::search when using raw pointers,
        // as the 4-argument overload is only available for ForwardIterators (not raw pointers).
