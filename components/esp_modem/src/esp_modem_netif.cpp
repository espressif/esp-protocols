    ...
    ppp_dte(std::move(e)), netif(ppp_netif)
{
    ...
    ESP_MODEM_THROW_IF_ESP_FAIL(esp_event_handler_register(NETIF_PPP_STATUS, ESP_EVENT_ANY_ID, &on_ppp_changed, (void *) this));
    ESP_MODEM_THROW_IF_ESP_FAIL(esp_event_handler_register(IP_EVENT, IP_EVENT_PPP_GOT_IP, esp_netif_action_connected, ppp_netif));
    ESP_MODEM_THROW_IF_ESP_FAIL(
        esp_event_handler_register(IP_EVENT, IP_EVENT_PPP_LOST_IP, esp_netif_action_disconnected, ppp_netif));
    ESP_MODEM_THROW_IF_ESP_FAIL(esp_netif_attach(ppp_netif, &driver));
}
