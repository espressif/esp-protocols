    ...
    : port(-1)
{
    ...
    ESP_MODEM_THROW_IF_ESP_FAIL(uart_param_config(config->port_num, &uart_config), "config uart parameter failed");
    ...
    ESP_MODEM_THROW_IF_ESP_FAIL(res, "config uart gpio failed");
    ...
    ESP_MODEM_THROW_IF_ESP_FAIL(res, "config uart flow control failed");
    ...
    res = uart_driver_install(config->port_num,
                              ...
                              0);
    ESP_MODEM_THROW_IF_ESP_FAIL(res, "install uart driver failed");
    ESP_MODEM_THROW_IF_ESP_FAIL(uart_set_rx_timeout(config->port_num, 1), "set rx timeout failed");

    ESP_MODEM_THROW_IF_ESP_FAIL(uart_set_rx_full_threshold(config->port_num, 64), "config rx full threshold failed");
