...
def analyze_results(report_path: Path) -> int:
    ...
    print(f"\n{'='*60}")
    print('Autobahn WebSocket Testsuite Results')
    print(f'Agent: {agent}')
    print(f"{'='*60}\n")
    ...
    print(
        f"  ✅ PASSED:         {counts['OK']:3d} "
        f"({counts['OK']/total*100:5.1f}%)",
    )
    print(
        f"  ⚠️  NON-STRICT:    {counts['NON-STRICT']:3d} "
        f"({counts['NON-STRICT']/total*100:5.1f}%)",
    )
    print(
        f"  ℹ️  INFORMATIONAL: {counts['INFORMATIONAL']:3d} "
        f"({counts['INFORMATIONAL']/total*100:5.1f}%)",
    )
    print(
        f"  ❌ FAILED:         {counts['FAILED']:3d} "
        f"({counts['FAILED']/total*100:5.1f}%)",
    )
    print(
        f"  ⭕ UNIMPLEMENTED:  {counts['UNIMPLEMENTED']:3d} "
        f"({counts['UNIMPLEMENTED']/total*100:5.1f}%)",
    )
    print()
    ...
    print(
        f"  {'Cat':<4} {'Pass':>6} {'N-S':>6} {'Info':>6} "
        f"{'Fail':>6} {'N/I':>6} {'Total':>6}",
    )
    print(
        f"  {'-'*4} {'-'*6} {'-'*6} {'-'*6} "
        f"{'-'*6} {'-'*6} {'-'*6}",
