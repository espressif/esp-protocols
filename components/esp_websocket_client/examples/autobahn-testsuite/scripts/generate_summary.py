...
CATEGORIES = {
    '1': {'name': 'Framing', 'critical': True, 'description': 'Basic frame structure'},
    '2': {'name': 'Ping/Pong', 'critical': True, 'description': 'Control frames'},
    '3': {'name': 'Reserved Bits', 'critical': True, 'description': 'RSV validation'},
    '4': {'name': 'Opcodes', 'critical': True, 'description': 'Valid/invalid opcodes'},
    '5': {'name': 'Fragmentation', 'critical': True, 'description': 'Message fragments'},
    '6': {'name': 'UTF-8', 'critical': False, 'description': 'Text validation'},
    '7': {'name': 'Close Handshake', 'critical': True, 'description': 'Connection closing'},
    '9': {'name': 'Performance', 'critical': False, 'description': 'Large messages'},
    '10': {'name': 'Miscellaneous', 'critical': True, 'description': 'Edge cases'},
    '12': {'name': 'Compression', 'critical': False, 'description': 'RFC 7692'},
    '13': {'name': 'Compression', 'critical': False, 'description': 'RFC 7692'},
}
...
def print_summary_table(summary):
    ...
    print()
    print(f"{Colors.GREEN}✅ PASSED:          {passed:3d}  ({pass_rate:5.1f}%){Colors.END}")
    print(f"{Colors.RED}❌ FAILED:          {failed:3d}  ({fail_rate:5.1f}%){Colors.END}")
    if non_strict > 0:
        print(f"{Colors.YELLOW}⚠️  NON-STRICT:       {non_strict:3d}  ({non_strict/total*100:5.1f}%){Colors.END}")
    if informational > 0:
        print(f"{Colors.CYAN}ℹ️  INFORMATIONAL:    {informational:3d}  ({informational/total*100:5.1f}%){Colors.END}")
    if unimplemented > 0:
        print(f"{Colors.MAGENTA}🔧 UNIMPLEMENTED:    {unimplemented:3d}  ({unimplemented/total*100:5.1f}%){Colors.END}")
    
    print()
    print(f"Overall Grade:      {grade_color}{Colors.BOLD}{grade}{Colors.END}")
    print()
    ...
    print(f"Embedded Client Rating: {rating_color}{Colors.BOLD}{rating}{Colors.END}")
    print()
...
def print_failed_tests(results, limit=20):
    ...
    print(f"Total Failed: {Colors.RED}{Colors.BOLD}{len(failed)}{Colors.END}\n")
...
def print_recommendations(summary):
    ...
    if '6' in by_category:
        ...
        if pass_rate < 50:
            print(f"{Colors.YELLOW}ℹ️  UTF-8 Validation (6.*): {pass_rate:.1f}% pass rate{Colors.END}")
            print(f"   This is acceptable for embedded clients in trusted environments.")
            print(f"   Consider adding UTF-8 validation if operating in untrusted networks.\n")
    
    print(f"{Colors.CYAN}💡 Next Steps:{Colors.END}\n")
    print(f"  1. Review failed tests in detail (see HTML report)")
    print(f"  2. Focus on critical categories (1-5, 7, 10)")
    print(f"  3. UTF-8 failures (category 6) are acceptable for embedded clients")
    print(f"  4. Compare your results with reference implementations\n")
...
<head>
    ...
    <style>
        ...
        .grade {{ font-size: 24px; font-weight: bold; padding: 10px 20px; border-radius: 5px; display: inline-block; }}
        ...
        .progress-bar {{ background: #ddd; border-radius: 10px; overflow: hidden; height: 30px; margin: 10px 0; }}
        .progress-fill {{ background: linear-gradient(90deg, #11998e 0%, #38ef7d 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; transition: width 0.3s; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card passed">
                <h3>{summary['by_behavior'].get('OK', 0)}</h3>
                <p>Passed ({summary['by_behavior'].get('OK', 0)/summary['total']*100:.1f}%)</p>
            </div>
            <div class="stat-card failed">
                <h3>{summary['by_behavior'].get('FAILED', 0)}</h3>
                <p>Failed ({summary['by_behavior'].get('FAILED', 0)/summary['total']*100:.1f}%)</p>
            </div>
        </div>
        
        <div class="progress-bar">
            <div class="progress-fill" style="width: {summary['by_behavior'].get('OK', 0)/summary['total']*100:.1f}%">
                {summary['by_behavior'].get('OK', 0)/summary['total']*100:.1f}% Pass Rate
            </div>
        </div>
        
        <div style="text-align: center; margin: 30px 0;">
            <span class="grade grade-{calculate_grade(summary['by_behavior'].get('OK', 0)/summary['total']*100)[0].lower()}">
...
"""
    ...
    print(f"{Colors.GREEN}✅ HTML summary generated: {output_file}{Colors.END}\n")
...
def main():
    ...
    if not reports_dir.exists():
        print(f"{Colors.RED}Error: Reports directory not found: {reports_dir}{Colors.END}")
        return 1
    
    print(f"{Colors.CYAN}📖 Parsing test results from: {reports_dir}{Colors.END}")
    ...
    print(f"{Colors.GREEN}✅ Parsed {len(results)} test results{Colors.END}")
