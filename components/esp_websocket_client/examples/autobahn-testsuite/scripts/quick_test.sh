...
# Backup original config (if it exists) before replacing with quick config
if [ -f config/fuzzingserver.json ]; then
    cp config/fuzzingserver.json config/fuzzingserver.json.backup
fi
