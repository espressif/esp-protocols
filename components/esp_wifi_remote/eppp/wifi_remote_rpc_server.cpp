...
static int s_sock = -1;
static RpcEngine s_rpc(eppp_rpc::role::SERVER);
...
static esp_err_t perform()
{
    ...
    switch (header.id) {
    case SET_MODE: {
        auto req = s_rpc.get_payload<wifi_mode_t>(SET_MODE, header);
        auto ret = esp_wifi_set_mode(req);
        if (s_rpc.send(SET_MODE, &ret) != ESP_OK) {
            return ESP_FAIL;
        }
        break;
    }
    case INIT: {
        auto req = s_rpc.get_payload<wifi_init_config_t>(INIT, header);
        req.osi_funcs = &g_wifi_osi_funcs;
        req.wpa_crypto_funcs = g_wifi_default_wpa_crypto_funcs;
        auto ret = esp_wifi_init(&req);
        if (s_rpc.send(INIT, &ret) != ESP_OK) {
            return ESP_FAIL;
        }
        break;
    }
...
esp_err_t RpcEngine::init_server()
{
    esp_err_t ret;
    ...
    ret = esp_tls_server_session_create(&cfg, s_sock, tls_);
    if (ret != ESP_OK) {
...
static void server(void *ctx)
{
    ...
    s_sock = accept(listen_sock, (struct sockaddr *)&source_addr, &addr_len);
    if (s_sock < 0) {
...
{
    s_ppp_netif = wifi_remote_eppp_init(EPPP_SERVER);
