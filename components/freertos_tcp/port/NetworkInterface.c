...
NetworkInterface_t *pxESP32_Eth_FillInterfaceDescriptor(BaseType_t xEMACIndex,
                                                        NetworkInterface_t *pxInterface)
{
    /* This function adds a network-interface. Make sure that the object pointed
     * to by 'pxInterface' is declared static or global, and that it will remain. */
    (void) xEMACIndex;

    memset(pxInterface, '\0', sizeof(*pxInterface));
    /* pcName is assigned by the caller (per-netif storage in esp_netif_stack). */
                                                    ...
                                                    BaseType_t xReleaseAfterSend)
{
    if ((pxDescriptor == NULL) || (pxDescriptor->pucEthernetBuffer == NULL) || (pxDescriptor->xDataLength == 0)) {
        ...
        /* FreeRTOS+TCP still expects the driver to release the buffer when asked. */
...
esp_err_t xESP32_Eth_NetworkInterfaceInput(NetworkInterface_t *pxInterface, void *buffer, size_t len, void *eb)
{
    ...
    /* Non-blocking: this runs on the Wi-Fi RX path; waiting would stall the driver. */
    const TickType_t xDescriptorWaitTime = 0;
    ...
    if ((buffer == NULL) || (len == 0) || (len > ipTOTAL_ETHERNET_FRAME_SIZE)) {
        ESP_LOGE(TAG, "Dropping invalid frame, len %u", (unsigned) len);
...
/* BufferAllocation_1.c entry point: allocate Ethernet frame storage for descriptors. */
