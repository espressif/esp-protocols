...
static void _mdns_browse_free(mdns_browse_t *browse);
...
void mdns_parse_packet(mdns_rx_packet_t *packet)
{
    ...
    char browse_result_hostname[MDNS_NAME_BUF_LEN] = {0};
    char browse_result_instance[MDNS_NAME_BUF_LEN] = {0};
    char browse_result_service[MDNS_NAME_BUF_LEN] = {0};
    char browse_result_proto[MDNS_NAME_BUF_LEN] = {0};
        ...
        while (content < (data + len)) {
            ...
            if (parsed_packet->discovery && _mdns_name_is_discovery(name, type)) {
                ...
                if (browse_result) {
                    ...
                    if (type == MDNS_TYPE_A || type == MDNS_TYPE_AAAA) {
                        if (!_str_null_or_empty(name->host)) {
                            memcpy(browse_result_hostname, name->host, MDNS_NAME_BUF_LEN);
                        }
                    }
                }
            }
...
static void _mdns_free_action(mdns_action_t *action)
{
    ...
    case ACTION_BROWSE_END:
        _mdns_browse_free(action->data.browse_add.browse);
        break;
...
/**
 * @brief  Free browse structure (except the results)
 */
static void _mdns_browse_free(mdns_browse_t *browse)
{
    free(browse->service);
...
static esp_err_t _mdns_browse_sync_result_find_or_add_result(mdns_browse_sync_t *sync_browse, mdns_result_t *r)
{
            ...
            if (r->ip_protocol == ip_protocol) {
                ...
                if (r->esp_netif == _mdns_get_esp_netif(tcpip_if) && !_str_null_or_empty(r->hostname) && !strcasecmp(hostname, r->hostname)) {
                    ...
                    if (!r_a) {
                        ...
                        if (_mdns_browse_sync_result_find_or_add_result(out_sync_browse, r) != ESP_OK) {
                            return;
                        }
                        break;
                    }
                                        ...
                                        uint32_t ttl, mdns_browse_sync_t *out_sync_browse)
{
    ...
    browse->result = r;
    _mdns_browse_sync_result_find_or_add_result(out_sync_browse, r);
    return;
...
{
    mdns_ip_addr_t *r_addr;
    mdns_ip_addr_t *prv = NULL;
    while (result_list) {
        if (!_str_null_or_empty(result_list->hostname) && !_str_null_or_empty(r->hostname) && !strcasecmp(result_list->hostname, r->hostname) &&
                result_list->ip_protocol == r->ip_protocol && result_list->addr) {
            // If there is a same hostname in previous hostname, we need to copy the address here.
            r_addr = result_list->addr;
            while (r_addr) {
                mdns_ip_addr_t *addr_add = _mdns_result_addr_create_ip(&(r_addr->addr));
                if (!addr_add) {
                    // no memory for adding the address, consider to free result and return false.
                    return ESP_ERR_NO_MEM;
                }
                if (!r->addr) {
                    r->addr = addr_add;
