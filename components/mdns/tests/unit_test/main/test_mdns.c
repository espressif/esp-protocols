...
TEST(mdns, add_remove_service)
{
    ...
    size_t misses = 0;
    for (int i = 0; i < txt_data_cout; ++i)
        for (int j = 0; j < txt_data_cout; ++j)
            if (strcmp(results->txt[i].key, txt_data[j].key) == 0) {
                TEST_ASSERT_EQUAL_STRING(results->txt[i].value, txt_data[j].value);
            } else {
                ++misses;
            };
    TEST_ASSERT_EQUAL(txt_data_cout, results->txt_count);
    TEST_ASSERT_EQUAL(txt_data_cout, misses);   // checks that we only missed the number of txt items
    mdns_query_results_free(results);
