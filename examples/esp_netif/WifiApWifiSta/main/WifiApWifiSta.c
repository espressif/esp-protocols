 ...
 *
 * WiFi AP + WiFi STA Example
 *
 * This example initializes a WiFi interface as an Access Point (with DHCP server)
 * and another WiFi interface as a Station. The WiFi AP provides network access to
...
static const char *TAG = "wifi_ap_wifi_sta";
...
/** Event handler for WiFi STA events */
                                   ...
                                   int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_DISCONNECTED:
            ...
            break;
        default:
            ESP_LOGW(TAG, "Unhandled Wi-Fi STA event: id=%ld", event_id);
            break;
        }
...
static void wifi_init_sta(void)
{
    wifi_config_t wifi_config = {
        .sta = {
            ...
            .password = CONFIG_EXAMPLE_WIFI_STA_PASS,
        },
    };
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));

    ESP_LOGI(TAG, "Wi-Fi STA initialized. SSID:%s password:%s", CONFIG_EXAMPLE_WIFI_STA_SSID, CONFIG_EXAMPLE_WIFI_STA_PASS);
}
...
void app_main(void)
{
    ...
    // Register event handlers - register only specific events to avoid warnings for unhandled events
    ...
    // Update WiFi AP DNS with WiFi STA DNS if enabled
#if CONFIG_EXAMPLE_WIFI_AP_DHCP_ENABLE_DNS
    ...
    if (esp_netif_get_dns_info(sta_netif, ESP_NETIF_DNS_MAIN, &sta_dns_info) == ESP_OK) {
        ...
        ap_dns_info.ip.u_addr.ip4 = sta_dns_info.ip.u_addr.ip4;
        esp_err_t err = esp_netif_set_dns_info(ap_netif, ESP_NETIF_DNS_MAIN, &ap_dns_info);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "Updated Wi-Fi AP DNS with WiFi STA DNS: " IPSTR, IP2STR(&sta_dns_info.ip.u_addr.ip4));
        }
    }
#endif // CONFIG_EXAMPLE_WIFI_AP_DHCP_ENABLE_DNS
