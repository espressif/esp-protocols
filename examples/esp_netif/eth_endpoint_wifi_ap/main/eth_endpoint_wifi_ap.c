 ...
 *
 * This example initializes an Ethernet interface as an endpoint (DHCP client toward
 * upstream) and a Wi-Fi interface as an access point (with DHCP server). The Ethernet
 * endpoint obtains an address from the wired network while the Wi-Fi AP provides
 * network access to wireless clients.
 */
#include <string.h>
...
static const char *TAG = "eth_endpoint_wifi_ap";
                                     ...
                                     int32_t event_id, void *event_data)
{
    ...
    xEventGroupSetBits(event_group, EXAMPLE_ETH_ENDPOINT_GOT_IP_BIT);
}
...
static esp_netif_t *wifi_init_ap(void)
{
    ...
    if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_IP_ADDR, &s_ap_ip_info.ip) != ESP_OK) {
        ESP_LOGE(TAG, "Invalid IP address: %s", CONFIG_EXAMPLE_WIFI_AP_IP_ADDR);
        return NULL;
    }
    if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_NETMASK, &s_ap_ip_info.netmask) != ESP_OK) {
        ESP_LOGE(TAG, "Invalid netmask: %s", CONFIG_EXAMPLE_WIFI_AP_NETMASK);
        return NULL;
    }
    if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_GW, &s_ap_ip_info.gw) != ESP_OK) {
    ...
    if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_DHCP_START_ADDR, (esp_ip4_addr_t *)&dhcp_lease.start_ip) != ESP_OK) {
        ESP_LOGW(TAG, "Invalid DHCP start address: %s, using default pool", CONFIG_EXAMPLE_WIFI_AP_DHCP_START_ADDR);
    } else if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_DHCP_END_ADDR, (esp_ip4_addr_t *)&dhcp_lease.end_ip) != ESP_OK) {
    ...
    if (err != ESP_OK) {
        ...
        if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_DHCP_DNS_MAIN, &dns_info.ip.u_addr.ip4) == ESP_OK) {
        ...
        if (esp_netif_str_to_ip4(CONFIG_EXAMPLE_WIFI_AP_DHCP_DNS_BACKUP, &dns_info.ip.u_addr.ip4) == ESP_OK) {
...
void app_main(void)
{
    ...
    xEventGroupWaitBits(event_group, EXAMPLE_ETH_ENDPOINT_GOT_IP_BIT, pdTRUE, pdTRUE, portMAX_DELAY);
    ...
    if (esp_netif_get_dns_info(eth_netif, ESP_NETIF_DNS_MAIN, &eth_dns_info) == ESP_OK) {
        ...
        if (err == ESP_OK) {
            ESP_LOGE(TAG, "Updated Wi-Fi AP DNS with Ethernet Station DNS: " IPSTR, IP2STR(&eth_dns_info.ip.u_addr.ip4));
        }
    }
#endif // CONFIG_EXAMPLE_WIFI_AP_DHCP_ENABLE_DNS
