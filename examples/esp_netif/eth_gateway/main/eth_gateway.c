 ...
 * on it (Ethernet gateway role). Devices connecting to the Ethernet port will
 * receive an IP address via DHCP (e.g. 192.168.5.x).
 */
#include <string.h>
...
static const char *TAG = "eth_gateway";
...
void app_main(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set DHCP lease time: %s", esp_err_to_name(err));
    }
