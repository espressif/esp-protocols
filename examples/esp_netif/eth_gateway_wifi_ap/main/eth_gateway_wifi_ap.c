 ...
 *
 * This example initializes the Ethernet interface as a gateway (DHCP server on the wired port)
 * and the Wi-Fi interface as an access point (DHCP server). Both interfaces provide network
 * access to connected devices simultaneously.
 */
#include <string.h>
...
static const char *TAG = "eth_gateway_wifi_ap";
                                              ...
                                              const char *ip_addr, const char *netmask, const char *gw)
{
    ...
    if (esp_netif_str_to_ip4(ip_addr, &ip_info->ip) != ESP_OK) {
                                               ...
                                               const char *start_addr, const char *end_addr)
{
    ...
    if (esp_netif_str_to_ip4(start_addr, (esp_ip4_addr_t *)&dhcp_lease.start_ip) != ESP_OK) {
        ESP_LOGW(TAG, "Invalid DHCP start address: %s, using default pool", start_addr);
    } else if (esp_netif_str_to_ip4(end_addr, (esp_ip4_addr_t *)&dhcp_lease.end_ip) != ESP_OK) {
...
static esp_err_t configure_dhcp_dns(esp_netif_t *netif, const char *dns_main, const char *dns_backup)
{
    ...
    if (dns_main && esp_netif_str_to_ip4(dns_main, &dns_info.ip.u_addr.ip4) == ESP_OK) {
    ...
    if (dns_backup && esp_netif_str_to_ip4(dns_backup, &dns_info.ip.u_addr.ip4) == ESP_OK) {
                              ...
                              int32_t event_id, void *event_data)
{
    ...
    case ETHERNET_EVENT_CONNECTED:
                 ...
                 mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
        {
            ...
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "DHCP server start: %s", esp_err_to_name(err));
            } else {
                ESP_LOGI(TAG, "DHCP server started on %s", esp_netif_get_desc(eth_netif));
            }
        }
        break;
    /* Optional: call esp_netif_dhcps_stop(eth_netif) here if link-down should tear down DHCP. */
...
/**
 * Configure Ethernet gateway network interface (netif) layer
 *
 * This function sets up the L3 (network layer) configuration for the Ethernet gateway:
 * - Parses IP configuration (IP address, netmask, gateway)
 * - Creates the Ethernet gateway netif with inherent configuration
 * - Configures DHCP server options (lease time, IP pool)
 * - Configures DNS servers (if enabled)
 * - Does not start the DHCP server (that happens on Ethernet link up).
 *
 * @return Pointer to the configured Ethernet gateway netif, or NULL on error
 */
static esp_netif_t *eth_gw_setup_netif(void)
{
    // Static IP configuration for Ethernet gateway
    static esp_netif_ip_info_t s_eth_gw_ip_info;
    esp_err_t err = parse_ip_config_from_strings(&s_eth_gw_ip_info,
                                                 CONFIG_EXAMPLE_ETH_GATEWAY_IP_ADDR,
                                                 CONFIG_EXAMPLE_ETH_GATEWAY_NETMASK,
                                                 CONFIG_EXAMPLE_ETH_GATEWAY_GW);
    if (err != ESP_OK) {
        return NULL;
    }
    ...
    configure_dhcp_server_options(eth_netif,
                                  CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_LEASE_TIME,
                                  CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_START_ADDR,
                                  CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_END_ADDR);

    // Configure DNS servers if enabled
#if CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_ENABLE_DNS
    configure_dhcp_dns(eth_netif, CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_DNS_MAIN, CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_DNS_BACKUP);
#endif // CONFIG_EXAMPLE_ETH_GATEWAY_DHCP_ENABLE_DNS

    ESP_LOGI(TAG, "Ethernet gateway netif configured. IP: " IPSTR ", netmask: " IPSTR ". DHCP starts when link is up.",
...
/**
 * Initialize and start Ethernet gateway
 *
 * This is the main entry point for Ethernet gateway initialization. It handles:
 ...
 * @return Pointer to the Ethernet gateway netif, or NULL on error
 */
static esp_netif_t *eth_gw_start(esp_eth_handle_t eth_handle)
{
    // Setup Ethernet gateway network interface (IP, DHCP DNS options; DHCP server starts on link up)
    esp_netif_t *eth_netif = eth_gw_setup_netif();
    if (eth_netif == NULL) {
        ESP_LOGE(TAG, "Failed to setup Ethernet gateway netif");
        return NULL;
    }
    ...
    // Print Ethernet gateway IP information
    print_ip_info(eth_netif, "Ethernet");
    ESP_LOGI(TAG, "Ethernet gateway started. Connect a device to the Ethernet port to get an IP via DHCP");
...
static wifi_auth_mode_t example_wifi_ap_authmode(void)
{
#if CONFIG_EXAMPLE_WIFI_AP_AUTHMODE_OPEN
    return WIFI_AUTH_OPEN;
#elif CONFIG_EXAMPLE_WIFI_AP_AUTHMODE_WPA2_PSK
    return WIFI_AUTH_WPA2_PSK;
#elif CONFIG_EXAMPLE_WIFI_AP_AUTHMODE_WPA_WPA2_PSK
    return WIFI_AUTH_WPA_WPA2_PSK;
#elif CONFIG_EXAMPLE_WIFI_AP_AUTHMODE_WPA3_PSK
    return WIFI_AUTH_WPA3_PSK;
#elif CONFIG_EXAMPLE_WIFI_AP_AUTHMODE_WPA2_WPA3_PSK
...
static esp_netif_t *wifi_ap_start(void)
{
    ...
    wifi_config_t ap_wifi_config = {
        .ap = {
            ...
            .max_connection = 4,
            .authmode = example_wifi_ap_authmode(),
            .pmf_cfg = {
                .required = false,
            },
        },
...
void app_main(void)
{
    ...
    // Initialize Ethernet gateway
    esp_netif_t *eth_netif = eth_gw_start(eth_handles[0]);
    if (eth_netif == NULL) {
        ESP_LOGE(TAG, "Failed to initialize Ethernet gateway");
        return;
    }
