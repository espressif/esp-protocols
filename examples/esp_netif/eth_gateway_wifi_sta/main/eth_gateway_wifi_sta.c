 ...
 *
 * This example initializes an Ethernet interface as a gateway (DHCP server on the wired port)
 * and a Wi-Fi interface as a station. The Ethernet gateway provides network access to
 * connected devices while the Wi-Fi STA connects to an external access point.
 */
#include <string.h>
...
static const char *TAG = "eth_gateway_wifi_sta";
